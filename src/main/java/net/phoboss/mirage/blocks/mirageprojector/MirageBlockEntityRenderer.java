...
public class MirageBlockEntityRenderer extends GeoBlockRenderer<MirageBlockEntity> {
    ...
    @Override
    public void renderFinal(MatrixStack poseStack, MirageBlockEntity blockEntity, BakedGeoModel model, VertexConsumerProvider bufferSource, VertexConsumer buffer, float tickDelta, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        super.renderFinal(poseStack, blockEntity, model, bufferSource, buffer, tickDelta, packedLight, packedOverlay, red, green, blue, alpha);
        ...
        if (mirageWorld != null) {
            ...
            poseStack.multiply(RotationAxis.of(pSpinAxis).rotationDegrees(time * pSpinSpeed * 0.05F - pSpinOffset)); // derived from BeaconBlockEntityRenderer: 45 deg/sec:2.25F (2.25/45=0.05) didn't bother to look into it more :)
            poseStack.translate(-pSpinPivot[0],-pSpinPivot[1],-pSpinPivot[2]);
            ...
            if(mwFrame != null) {
                ...
                poseStack.multiply(RotationAxis.of(pSpinAxisFrame).rotationDegrees(time * pSpinSpeedFrame * 0.05F - pSpinOffsetFrame));
                poseStack.translate(-pSpinPivotFrame[0],-pSpinPivotFrame[1],-pSpinPivotFrame[2]);
            }
            ...
            mirageWorld.render(projectorPos, tickDelta, poseStack, bufferSource, packedLight, 0);
